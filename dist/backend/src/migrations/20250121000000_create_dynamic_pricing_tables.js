...
async function insertBaseData(db) {
    ...
    for (const price of basePrices) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `, price.product_type, price.format, price.paper_type, price.paper_density, price.lamination, price.base_price, price.urgency);
    }
    // Цены на материалы
    ...
    for (const material of materialPrices) {
      ...
      VALUES (?, ?, ?, ?, ?)
    `, material.material_name, material.material_type, material.unit, material.price_per_unit, material.supplier);
    }
    // Цены на услуги
    const servicePrices = [
    ...
    for (const multiplier of multipliers) {
      ...
      VALUES (?, ?, ?, ?)
    `, multiplier.multiplier_type, multiplier.multiplier_name, multiplier.multiplier_value, multiplier.description);
    }
    // Правила скидок
    const discountRules = [
    ...
    for (const rule of discountRules) {
      ...
      VALUES (?, ?, ?, ?, ?)
    `, rule.discount_type, rule.discount_name, rule.min_quantity, rule.discount_percent, rule.conditions);
    }
    ...
    VALUES (?, ?, ?, ?, ?)
  `, aiModelConfig.model_name, aiModelConfig.model_type, aiModelConfig.model_parameters, aiModelConfig.accuracy, aiModelConfig.confidence_threshold);
}
