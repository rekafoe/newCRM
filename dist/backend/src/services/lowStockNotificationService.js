...
exports.LowStockNotificationService = void 0;
...
class LowStockNotificationService {
      ...
      `);
          ...
          `, material.id);
            ...
            `, material.id, currentQuantity, minQuantity, alertLevel);
                        ...
                        logger_1.logger.warn('Создано уведомление о низком остатке', {
                            materialId: material.id,
                            materialName: material.name,
                            currentQuantity,
                            minQuantity,
            ...
            logger_1.logger.info('Проверка остатков завершена', {
                totalAlerts,
                criticalAlerts,
                warningAlerts,
                outOfStockAlerts
    ...
    static async resolveAlert(alertId, userId) {
      ...
      `, userId, alertId);
            logger_1.logger.info('Уведомление отмечено как решенное', { alertId, userId });
            return true;
        }
        catch (error) {
    ...
    static async autoResolveAlerts() {
      ...
      `);
          ...
          `, alert.id);
            ...
            if (resolvedCount > 0) {
                logger_1.logger.info('Автоматически решены уведомления', { resolvedCount });
            }
     ...
     */
    static async scheduleStockCheck() {
        // Этот метод можно расширить для интеграции с cron или другими планировщиками
        logger_1.logger.info('Запланирована проверка остатков материалов');
    }
}
exports.LowStockNotificationService = LowStockNotificationService;
