...
class UserNotificationService {
    ...
    static async getAllUsers() {
      ...
      SELECT 
        ...
        COALESCE(notification_preferences, '{"low_stock": true, "new_orders": true, "system_alerts": true}') as notification_preferences
      FROM users
     ...
     */
    static async sendLowStockAlert(materialName, currentQuantity, minStock, supplierName) {
        const notification = {
     ...
     */
    static async sendNewOrderAlert(orderId, customerName, totalAmount) {
        const notification = {
            title: '📋 Новый заказ',
     ...
     */
    static async sendSystemAlert(title, message, priority = 'medium') {
        const notification = {
            title,
            message,
            priority,
     ...
     */
    static async sendTelegramMessage(chatId, notification) {
        try {
     ...
     */
    static formatLowStockMessage(materialName, currentQuantity, minStock, supplierName) {
     ...
     */
    static formatNewOrderMessage(orderId, customerName, totalAmount) {
     ...
     */
    static async updateUserTelegramChatId(userId, chatId) {
        const db = await (0, db_1.getDb)();
        try {
            await db.run(`
        UPDATE users 
        ...
        WHERE id = ?
      `, chatId, userId);
            console.log(`✅ Updated Telegram chat_id for user ${userId}`);
            return true;
        }
        catch (error) {
     ...
     */
    static async sendTestMessageToBotUsers() {
...
UserNotificationService.botToken = '7954389446:AAHmFQmGJOp-fZLhaZD9ZGxvrJ_WEyKMtnE';
UserNotificationService.isInitialized = false;
