...
class DynamicPricingService {
    ...
    static async getMinimumOrderCosts() {
        try {
            ...
            logger_1.logger.info('Получены минимальные стоимости заказов', { count: staticCosts.length });
            return staticCosts;
        }
        catch (error) {
    ...
    static async getMinimumCostForOrder(format, productType, quantity) {
        try {
            const costs = await this.getMinimumOrderCosts();
            const cost = costs.find(c => c.format === format &&
    ...
    static async getProductBasePrices(filters) {
        try {
            ...
            logger_1.logger.info('Получены базовые цены продуктов', { count: filteredPrices.length, filters });
    ...
    static async getMaterialPrices() {
      ...
      `);
            ...
            logger_1.logger.info('Получены цены на материалы из основного склада', { count: materialPrices.length });
            ...
            logger_1.logger.warn('Используем статические данные материалов', { count: staticPrices.length });
            return staticPrices;
        }
    }
    // Получение цен на услуги
    static async getServicePrices() {
        try {
            // Пока возвращаем статические данные
            const staticPrices = [
            ...
            logger_1.logger.info('Получены цены на услуги', { count: staticPrices.length });
            return staticPrices;
        }
        catch (error) {
    ...
    static async getPricingMultipliers(multiplierType) {
        try {
            // Пока возвращаем статические данные
            ...
            logger_1.logger.info('Получены коэффициенты ценообразования', { count: filteredMultipliers.length, multiplierType });
    ...
    static async getDiscountRules(discountType) {
        try {
            // Пока возвращаем статические данные
            const staticRules = [
            ...
            logger_1.logger.info('Получены правила скидок', { count: filteredRules.length, discountType });
    ...
    static async getAIModelConfig(modelName) {
        try {
            // Пока возвращаем статические данные
            const staticConfig = {
                id: 1,
            ...
            logger_1.logger.info('Получена конфигурация ИИ-модели', { modelName: staticConfig.model_name });
            return staticConfig;
        }
        catch (error) {
    ...
    static async updateMinimumOrderCost(id, updates) {
        try {
            // Пока просто логируем обновление
            logger_1.logger.info('Обновлена минимальная стоимость заказа', { id, updates });
            return true;
        }
        catch (error) {
    ...
    static async updateProductBasePrice(id, updates) {
        try {
            // Пока просто логируем обновление
            logger_1.logger.info('Обновлена базовая цена продукта', { id, updates });
            return true;
        }
        catch (error) {
    ...
    static async updateMaterialPrice(id, updates) {
        try {
            // Пока просто логируем обновление
            logger_1.logger.info('Обновлена цена материала', { id, updates });
            return true;
        }
        catch (error) {
    ...
    static async updateServicePrice(id, updates) {
        try {
            // Пока просто логируем обновление
            logger_1.logger.info('Обновлена цена услуги', { id, updates });
            return true;
        }
        catch (error) {
    ...
    static async updatePricingMultiplier(id, updates) {
        try {
            // Пока просто логируем обновление
            logger_1.logger.info('Обновлен коэффициент ценообразования', { id, updates });
            return true;
        }
        catch (error) {
    ...
    static async updateDiscountRule(id, updates) {
        try {
            // Пока просто логируем обновление
            logger_1.logger.info('Обновлено правило скидки', { id, updates });
            return true;
        }
        catch (error) {
    ...
    static async updateAIModelConfig(id, updates) {
        try {
            // Пока просто логируем обновление
            logger_1.logger.info('Обновлена конфигурация ИИ-модели', { id, updates });
            return true;
        }
        catch (error) {
    ...
    static async exportPricingData() {
        try {
            const [minimumOrderCosts, productBasePrices, materialPrices, servicePrices, pricingMultipliers, discountRules, aiModelConfigs] = await Promise.all([
            ...
            logger_1.logger.info('Экспортированы данные ценообразования', {
