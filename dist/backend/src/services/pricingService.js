...
class PricingService {
    static async calculateProductPrice(params) {
        ...
        const tierMultiplier = this.PRICING_MULTIPLIERS[pricingType];
    ...
    static async calculateMaterials(productType, specifications, quantity) {
        ...
        if (specifications.lamination && specifications.lamination !== 'none') {
            ...
            const laminationPrice = this.MATERIAL_PRICES[laminationName] || 0.03;
            const sheetsPerItem = this.calculateSheetsPerItem(productType, specifications);
    ...
    static async calculateServices(productType, specifications, quantity) {
        const services = [];
        // Печать
        const sheetsPerItem = this.calculateSheetsPerItem(productType, specifications);
        const totalSheets = Math.ceil(quantity * sheetsPerItem * 1.02);
    ...
    static calculateSheetsPerItem(productType, specifications) {
        if (productType === 'Листовки') {
            const format = specifications.format || 'A6';
            const upOnSra3 = {
                'A6': 8,
    ...
    static calculateCutsPerItem(productType, specifications) {
        if (productType === 'Листовки') {
            const format = specifications.format || 'A6';
            const cutsPerSra3 = {
    ...
    static calculateInkUsage(productType, specifications) {
        ...
        const inkPerSheet = baseInkPerSheet[format] || 0.1;
        return inkPerSheet * sides;
    }
    static calculateVolumeDiscount(quantity) {
    ...
    static calculateLoyaltyDiscount(customerType) {
        if (customerType === 'vip')
            return 15;
...
PricingService.MATERIAL_PRICES = {
