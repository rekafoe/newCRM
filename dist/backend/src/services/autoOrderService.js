...
exports.AutoOrderService = void 0;
...
class AutoOrderService {
     ...
     */
    static async createAutoOrder(materialIds) {
        ...
        let materialsQuery = `
      SELECT 
        m.id, m.name, m.quantity, m.min_quantity, m.min_stock_level, m.sheet_price_single,
    ...
    `;
        if (materialIds && materialIds.length > 0) {
            ...
            materialsQuery += ` AND m.quantity <= COALESCE(m.min_stock_level, m.min_quantity, 10)`;
        }
     ...
     */
    static async createSupplierOrder(supplierId, materials) {
        const db = await (0, db_1.getDb)();
        ...
        if (this.config.notificationEnabled) {
            ...
            await telegramService_1.TelegramService.sendOrderNotification(notification);
        }
     ...
     */
    static async saveAutoOrder(order) {
        const db = await (0, db_1.getDb)();
     ...
     */
    static async getAutoOrders(status) {
        const db = await (0, db_1.getDb)();
        let query = `
      SELECT 
...
exports.AutoOrderService = AutoOrderService;
AutoOrderService.config = {
    enabled: false,
