...
exports.MaterialAlertService = void 0;
...
class MaterialAlertService {
    ...
    static async getAllAlerts(filters) {
        ...
        if (is_read !== undefined) {
            where.push('ma.is_read = ?');
            params.push(is_read ? 1 : 0);
        }
        if (alert_type) {
            where.push('ma.alert_type = ?');
            params.push(alert_type);
        }
        if (material_id) {
            where.push('ma.material_id = ?');
        ...
        const alerts = await db.all(`SELECT 
    ...
    static async markAsRead(alertId, userId) {
        const db = await (0, database_1.getDb)();
        await db.run('UPDATE material_alerts SET is_read = 1, read_at = CURRENT_TIMESTAMP, user_id = ? WHERE id = ?', userId || null, alertId);
    }
    // Отметить все уведомления как прочитанные
    static async markAllAsRead(userId) {
        const db = await (0, database_1.getDb)();
        await db.run('UPDATE material_alerts SET is_read = 1, read_at = CURRENT_TIMESTAMP, user_id = ? WHERE is_read = 0', userId || null);
    }
    // Удалить уведомление
    ...
    static async deleteOldAlerts() {
        const db = await (0, database_1.getDb)();
        await db.run('DELETE FROM material_alerts WHERE created_at < datetime("now", "-30 days")');
    }
    // Проверить и создать уведомления о низких остатках
    static async checkLowStockAlerts() {
        const db = await (0, database_1.getDb)();
        // Получаем материалы с низкими остатками
        const lowStockMaterials = await db.all(`SELECT 
       ...
       AND m.id NOT IN (
        ...
        for (const material of lowStockMaterials) {
            ...
            const result = await db.run('INSERT INTO material_alerts (material_id, alert_type, current_quantity, threshold_quantity, message) VALUES (?, ?, ?, ?, ?)', material.id, alertType, material.quantity, material.min_quantity, message);
            alerts.push({
                id: result.lastID,
...
exports.MaterialAlertService = MaterialAlertService;
