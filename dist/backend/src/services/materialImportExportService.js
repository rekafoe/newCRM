...
exports.MaterialImportExportService = void 0;
            ...
            for (let i = 0; i < data.materials.length; i++) {
                ...
                try {
                    ...
                    if (material.category_name) {
                        const category = await db.get('SELECT id FROM material_categories WHERE name = ?', material.category_name);
                        categoryId = category?.id || null;
                    }
                    // Находим поставщика по имени
                    let supplierId = null;
                    if (material.supplier_name) {
                        const supplier = await db.get('SELECT id FROM suppliers WHERE name = ?', material.supplier_name);
                        supplierId = supplier?.id || null;
                    }
                    // Создаем материал
                    const result = await db.run('INSERT INTO materials (name, unit, quantity, min_quantity, sheet_price_single, category_id, supplier_id) VALUES (?, ?, ?, ?, ?, ?, ?)', material.name, material.unit, material.quantity, material.min_quantity || null, material.sheet_price_single || null, categoryId, supplierId);
                    results.push({
                        row: i + 1,
                        name: material.name,
                        id: result.lastID,
...
exports.MaterialImportExportService = MaterialImportExportService;
