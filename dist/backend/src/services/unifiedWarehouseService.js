...
exports.UnifiedWarehouseService = void 0;
...
class UnifiedWarehouseService {
    ...
    static async getAllMaterials() {
      ...
      `);
            ...
            const unifiedMaterials = materials.map((material) => {
            ...
            logger_1.logger.info('Получены унифицированные материалы', { count: unifiedMaterials.length });
            return unifiedMaterials;
        }
        catch (error) {
            logger_1.logger.error('Ошибка получения унифицированных материалов', error);
            return [];
        }
    }
    // Получить статистику склада
    static async getWarehouseStats() {
        try {
            ...
            const totalValue = materials.reduce((sum, m) => sum + (m.quantity * (m.price_per_unit || m.sheet_price_single || 0)), 0);
            const reservedValue = materials.reduce((sum, m) => sum + ((m.reserved_quantity || 0) * (m.price_per_unit || m.sheet_price_single || 0)), 0);
            const availableValue = materials.reduce((sum, m) => sum + ((m.available_quantity || 0) * (m.price_per_unit || m.sheet_price_single || 0)), 0);
            ...
            logger_1.logger.info('Получена статистика склада', stats);
            return stats;
        }
        catch (error) {
    ...
    static async reserveMaterials(reservations) {
        ...
        `, reservation.material_id);
          ...
          VALUES (?, ?, ?, 'reserved', ?, ?)
        `, reservation.material_id, reservation.order_id || null, reservation.quantity, reservation.reason || 'Резерв для заказа', expiresAt.toISOString());
            ...
            await db.run('COMMIT');
            logger_1.logger.info('Материалы зарезервированы', { count: createdReservations.length });
    ...
    static async confirmReservations(reservationIds) {
        try {
            const db = await (0, database_1.getDb)();
            await db.run('BEGIN');
        ...
        `, reservation.quantity, reservation.material_id);
          ...
          VALUES (?, ?, ?, ?, ?)
        `, reservation.material_id, -reservation.quantity, reservation.reason || 'Списание по заказу', reservation.order_id, null // user_id можно получить из контекста
                );
        ...
        `, reservationId);
            ...
            await db.run('COMMIT');
            logger_1.logger.info('Резервы подтверждены', { count: reservationIds.length });
        }
        catch (error) {
            // await db.run('ROLLBACK');
    ...
    // Отмена резерва
    static async cancelReservations(reservationIds) {
        try {
            const db = await (0, database_1.getDb)();
            await db.run(`
        UPDATE material_reservations 
      ...
      `, ...reservationIds);
            logger_1.logger.info('Резервы отменены', { count: reservationIds.length });
        }
        catch (error) {
    ...
    static async getReservationsByOrder(orderId) {
        try {
            const db = await (0, database_1.getDb)();
    ...
    static async syncWithPricing() {
      ...
      `);
            logger_1.logger.info('Синхронизация с системой ценообразования завершена');
        }
        catch (error) {
...
exports.UnifiedWarehouseService = UnifiedWarehouseService;
