...
router.get('/daily/:date/summary', (0, middleware_1.asyncHandler)(async (req, res) => {
    ...
    const ordersCount = await db.get(`SELECT COUNT(1) as c FROM orders WHERE substr(createdAt,1,10) = ?`, d);
    const sums = await db.get(`SELECT 
    ...
    const prepay = await db.get(`SELECT 
    ...
    const materials = await db.all(`SELECT m.id as materialId, m.name as material_name,
    ...
    const debtInfo = await db.get(`SELECT 
...
router.get('/materials/top', (0, middleware_1.asyncHandler)(async (req, res) => {
    ...
    const db = await (0, database_1.getDb)();
    const rows = await db.all(`SELECT m.id, m.name, SUM(CASE WHEN mm.delta < 0 THEN -mm.delta ELSE 0 END) AS spent
       FROM material_moves mm
       JOIN materials m ON m.id = mm.materialId
      ${whereSql}
      GROUP BY m.id, m.name
...
router.get('/materials/forecast', (0, middleware_1.asyncHandler)(async (req, res) => {
    const db = await (0, database_1.getDb)();
    const rows = await db.all(`SELECT m.id, m.name, m.unit, m.quantity, m.min_quantity,
